<script lang="ts">
	...
	$: monthlyIncome =
		24 * 30 * ($landsIncomeStore + $miningPowerStore * $rplanetPrices.AETHER * $waxPrice);

	$: currentHoldings = Balance.getAccountBalances($currencyBalance, $rplanetPrices, $waxPrice).usd;
</script>
...
<main>
	<nav class="section level has-text-center">
		...
		<div class="level-item has-text-centered">
			<div class="pb-2 has-tooltip-bottom has-tooltip-info" data-tooltip="You are a {rank.name}">
